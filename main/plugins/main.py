...
async def _360(event):
    ...
    if not os.path.isdir("encodemedia"):
        await event.delete()
        os.mkdir("encodemedia")
        cmd = '-vf scale=640x360 -b:v 276k -quality good -speed 4 -crf 18'
        await encode(event, msg, cmd)
        os.rmdir("encodemedia")
    else:
        await event.edit(f"Another process in progress!\n\n**[LOG CHANNEL](https://t.me/{LOG_CHANNEL})**", link_preview=False)
...
async def _240(event):
    ...
    if not os.path.isdir("encodemedia"):
        await event.delete()
        os.mkdir("encodemedia")
        cmd = '-vf scale=320x240 -b:v 196k -quality good -speed 4 -crf 18'
        await encode(event, msg, cmd)
        os.rmdir("encodemedia")
    else:
        await event.edit(f"Another process in progress!\n\n**[LOG CHANNEL](https://t.me/{LOG_CHANNEL})**", link_preview=False)
...
async def _720(event):
    ...
    if not os.path.isdir("encodemedia"):
        await event.delete()
        os.mkdir("encodemedia")
        cmd = '-vf scale=1280x720 -b:v 1024k -quality good -speed 4 -crf 18'
        await encode(event, msg, cmd)
        os.rmdir("encodemedia")
    else:
        await event.edit(f"Another process in progress!\n\n**[LOG CHANNEL](https://t.me/{LOG_CHANNEL})**", link_preview=False)
