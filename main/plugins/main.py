...
async def _240(event):
    ...
    if not os.path.isdir("compressmedia"):
        ...
        cmd = '-vcodec libx264 -crf 28 -acodec copy -vf "scale=320:240" -preset ultrafast'
        await compress(event, msg, cmd)
...
async def _360(event):
    ...
    if not os.path.isdir("compressmedia"):
        await event.delete()
        os.mkdir("compressmedia")
        cmd = '-vcodec libx264 -crf 28 -acodec copy -vf "scale=640:360" -preset ultrafast'
        await compress(event, msg, cmd)
        os.rmdir("compressmedia")
    else:
        await event.edit(f"Another process in progress!\n\n**[LOG CHANNEL](https://t.me/{LOG_CHANNEL})**", link_preview=False)
...
async def _480(event):
    ...
    if not os.path.isdir("compressmedia"):
        await event.delete()
        os.mkdir("compressmedia")
        cmd = '-vcodec libx264 -crf 28 -acodec copy -vf "scale=854:480" -preset ultrafast'
        await compress(event, msg, cmd)
        os.rmdir("compressmedia")
    else:
        await event.edit(f"Another process in progress!\n\n**[LOG CHANNEL](https://t.me/{LOG_CHANNEL})**", link_preview=False)
